...
import type { ContractItem, Quotation, SimpleContractItem } from "../config/types";
...
const AddContractTable = () => {
    ...
    const idString = useMemo(() => {
        return displayProducts.map((item: any) => item.quotationItemId).join(',');
    }, [displayProducts]);

    const { data: displayedProducts } = useQuery<ContractItem[]>({
        queryKey: ['items-detail-contract', page, size, idString],
        queryFn: async () => {
            try {
                const response = await axiosClient.post(`/api/contract/items-detail`, displayProducts);
    ...
    return (
        <Box>
            ...
            <div className="table-scroll-container" style={{ flex: 1, overflowY: 'auto', marginTop: 1, height: 'calc(100vh - 150px)' }}>
...
export default React.memo(AddContractTable);